...
Makefile Commands
CommandWhat it doesmake fetchFetch fresh odds, stats, and injuries (run all three)make oddsFetch odds onlymake statsFetch stats only (requires odds to exist first)make injuriesFetch injuries only (requires odds to exist first)make todayList today's gamesmake gamesList all upcoming gamesmake query Q="Team vs Team"Run a full analysis on a matchupmake resetWipe the database and start fresh
